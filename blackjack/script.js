var Card = /** @class */ (function () {
    function Card(value, src) {
        this.backside_src = '1B';
        this.file_path_prefix = 'cards/';
        this.file_path_suffix = '.svg';
        this.value = value;
        this.src = src;
    }
    Card.prototype.getImageFilePath = function (isVisible) {
        if (isVisible) {
...
var CardStack = /** @class */ (function () {
    function CardStack(fullDeck, shuffleAfter, cards) {
        if (fullDeck === void 0) { fullDeck = false; }
        if (shuffleAfter === void 0) { shuffleAfter = true; }
    ...
    CardStack.prototype.add = function (card, shuffleAfter) {
        if (shuffleAfter === void 0) { shuffleAfter = true; }
        var new_card = card;
    ...
    CardStack.prototype.remove = function (shuffleBefore) {
        if (shuffleBefore === void 0) { shuffleBefore = true; }
    ...
    CardStack.prototype.shuffle = function () {
        if (this.cards.length > 0) {
            var rand = void 0, rand_card = void 0;
    ...
    CardStack.prototype.value = function () {
        var result = 0;
        var aces = 0;
...
var Player = /** @class */ (function () {
    function Player(id, visible, hand) {
        if (hand === void 0) { hand = new CardStack(); }
        this.last_bet = 0;
        this.can_draw = true;
    ...
    Player.prototype.bet = function (ammount, doubled) {
        if (doubled === void 0) { doubled = false; }
        if (ammount < 0) {
            ammount = 0;
            console.log('use gain to add chips');
        }
    ...
    Player.prototype.gain = function (ammount) {
        if (ammount < 0) {
            ammount = 0;
            console.log('use bet to remove chips');
        }
        this.last_bet = 0;
        this.chips += ammount;
        localStorage.setItem('chips', String(this.chips));
...
function inputHandler(player, cmd, div) {
    if (cmd == 'bet') {
        var inputField = document.getElementById('betInput');
        if (inputField != null) {
        ...
        if (cmd == 'stand') {
            ...
            if (player.hand.cards.length == 2 && player.hand.value() == 21) {
                ...
                if (gameResultGraph != null) {
                    gameResultGraph.innerHTML = 'Blackjack!';
                }
                var payout = player.last_bet + (player.last_bet * 3) / 2;
                play1.gain(payout);
                ...
                console.log('Bust');
                if (gameResultGraph != null) {
                    gameResultGraph.innerHTML = 'Bust...';
                }
                setTimeout(function () { location.reload(); }, gameOverTimeout);
            }
            else {
        ...
        if (cmd == 'double') {
            if (player.chips >= player.last_bet || !player.can_double) {
                ...
                var but = document.getElementById('doubleButton');
                if (but != null) {
                    but.disabled = true;
                }
            }
...
function handleDealer(player_value) {
    if (deal.hand.value() < 17) {
        ...
        if (deal.hand.value() > 21) {
            console.log('You win');
            if (gameResultGraph != null) {
                gameResultGraph.innerHTML = 'You Win.';
            }
            ...
            if (deal.hand.value() > player_value) {
                ...
                if (gameResultGraph != null) {
                    gameResultGraph.innerHTML = 'Stand Off.';
                }
